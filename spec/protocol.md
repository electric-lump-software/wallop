...
- **Weather observation window.** The execution receipt's `weather_observation_time` MUST fall in the interval `[lock.weather_time, lock.weather_time + 3600s]` inclusive. Verifiers MUST reject on violation. This prevents an infrastructure-level attacker from fetching weather from any point in time and retroactively attributing it to the draw's declared window, which would otherwise allow the entropy-retry manipulation described as a residual trust assumption in §4.4.
