defmodule WallopWeb.OperatorControllerTest do
  ...
  describe "GET /operator/:slug/keys" do
    ...
    test "returns each operator key with key_id, public_key_hex, valid_from, inserted_at, key_class",
         %{conn: conn} do
      ...
      assert is_binary(key["valid_from"])
    ...
    test "inserted_at and valid_from match the spec §4.2.1 canonical RFC 3339 form",
         %{conn: conn} do
      ...
      assert key["valid_from"] =~ canonical,
             "valid_from #{inspect(key["valid_from"])} must match canonical RFC 3339 form"
    end
