(() => {
  ...
  var closure, globalSelf, phxWindow, global, DEFAULT_VSN, SOCKET_STATES, DEFAULT_TIMEOUT, WS_CLOSE_NORMAL, CHANNEL_STATES, CHANNEL_EVENTS, TRANSPORTS, XHR_STATES, AUTH_TOKEN_PREFIX, Push, Timer, Channel, Ajax, arrayBufferToBase64, LongPoll, serializer_default, Socket;
...
`
      ...
      Rendered = class {
        ...
        mergeDiff(diff) {
          ...
          if (newc) {
            ...
            for (const cid in newc) {
              newc[cid] = this.cachedFindComponent(cid, newc[cid], oldc, newc, cache2);
            }
            for (const cid in newc) {
              oldc[cid] = newc[cid];
            }
        ...
        cachedFindComponent(cid, cdiff, oldc, newc, cache2) {
          if (cache2[cid]) {
            return cache2[cid];
          } else {
            ...
            if (isCid(scid)) {
              let tdiff;
              if (scid > 0) {
                tdiff = this.cachedFindComponent(scid, newc[scid], oldc, newc, cache2);
              } else {
                tdiff = oldc[-scid];
              }
              stat = tdiff[STATIC];
    ...
    "js/verify.js"() {
      ...
      VerifyRunner = class {
        ...
        async run() {
          ...
          this.box.innerHTML = "";
          anime_es_default({ targets: this.box, opacity: [0, 1], duration: 300, easing: "easeOutCubic" });
        ...
        countUp(el, target, duration) {
          ...
          return new Promise((resolve) => {
            anime_es_default({
              ...
              easing: "easeOutCirc",
              update: () => {
                el.textContent = `${counter.val}/${target}`;
              },
              complete: resolve
            });
          });
    ...
    "js/app.js"() {
      ...
      Hooks2.Countdown = {
        ...
        start() {
          this.stop();
          this.tick();
          this._interval = setInterval(() => this.tick(), 1e3);
        },
        stop() {
          if (this._interval) {
        ...
        tick() {
          ...
          let diff = Math.max(0, Math.floor((target - Date.now()) / 1e3));
      ...
      Hooks2.DrawReveal = {
        mounted() {
          ...
          this.activeSteps.push(() => {
            hook.pushEvent("reveal_complete", {});
          });
          this.currentStep = 0;
          if (this.mode === "auto") {
            this.runSequence();
        ...
        buildSteps() {
          ...
          function showDetail(idx) {
            ...
            if (detail && text) {
              ...
              if (hashMatch) {
                ...
                if (countMatch) {
                  ...
                  anime_es_default({
                    ...
                    easing: "easeOutCirc",
                    update: () => {
                      detail.textContent = prefix + counter.val + suffix;
                    }
                  });
                } else {
              ...
              anime_es_default({ targets: detail, translateY: [-12, 0], opacity: [0, 1], duration: 800, easing: "easeOutElastic(1, .8)" });
            }
          }
          this.steps = [
            // Stage 0: activate
      ...
      document.addEventListener("click", (e) => {
        ...
        let end = target.getBoundingClientRect().top + window.scrollY - 80;
        let scroll = { y: start };
        anime_es_default({
          targets: scroll,
          y: end,
