defmodule WallopCore.Repo.Migrations.RemoveEntriesJsonbColumn do
  @moduledoc """
  Drops the JSONB `entries` column from draws. Entry data now lives
  in the `entries` table. Updates the immutability trigger to remove
  the entries column reference (entry immutability is enforced by
  the entries table trigger).
  """
  use Ecto.Migration

  def up do
    # Update immutability trigger — remove entries column from committed fields check
    execute """
